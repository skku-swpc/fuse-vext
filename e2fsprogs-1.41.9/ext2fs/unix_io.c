			      ...
			      int count, void *buf)
{
	...
	actual = virt_read(data->dev, buf, size);
	if (actual != size) {
		if (actual < 0)
			actual = 0;
...
#define BLOCKALIGN 512
	...
	if (alignsize) {
		...
		actual = virt_read(data->dev, buf, alignsize);
		if (actual != alignsize)
			goto short_read;
	}
	if (fragment) {
		void* next=(void*)((unsigned long)buf+alignsize);
		//actual = read(data->dev, sector, BLOCKALIGN);
		actual = virt_read(data->dev, next, BLOCKALIGN);
			       ...
			       int count, const void *buf)
{
	...
	actual = virt_write(data->dev, buf, size);
	if (actual != size) {
...
#endif
	data->dev=virt_open(io->name, open_flags);	
				 ...
				 int size, const void *buf)
{
	...
	if (virt_lseek(data->dev, offset + data->offset, SEEK_SET) < 0)
		return errno;
